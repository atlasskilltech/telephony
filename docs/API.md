...
| GET `/dashboard/call-analytics?from=&to=` | call-QA analytics for a window (ISO `from`/`to`; defaults to last 30 days; `from=` empty ⇒ all-time). Returns metrics (+deltas vs previous equal window), score trend & volume, sentiment split, QA radar, agent leaderboard, recent calls |
